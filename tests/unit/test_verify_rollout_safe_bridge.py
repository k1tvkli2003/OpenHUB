...
async def test_trigger_restart_uses_configured_kubectl_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = verify_script.VerifyConfig(
        ...
        rollout_namespace="openhub-e2e",
        rollout_context="kind-openhub-local",
        rollout_target="statefulset/openhub-e2e-workload",
        delete_owner_after_first_turn=False,
        postgresql_pod="postgresql-0",
        postgresql_user="openhub",
        postgresql_database="openhub",
    ...
    assert seen["cmd"] == [
        ...
        "rollout",
        "restart",
        "statefulset/openhub-e2e-workload",
    ]
