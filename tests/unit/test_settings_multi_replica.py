...
def test_settings_http_downstream_transport_policy_from_env(monkeypatch, policy: str):
    monkeypatch.setenv("OPENHUB_HTTP_DOWNSTREAM_TRANSPORT_POLICY", policy)
    settings = Settings()
...
def test_settings_conversation_archive_queue_max_bytes_from_env(monkeypatch):
    monkeypatch.setenv("OPENHUB_CONVERSATION_ARCHIVE_QUEUE_MAX_BYTES", "16777216")
    settings = Settings()
...
def test_settings_circuit_breaker_tuning_env_overrides_are_removed(monkeypatch):
    ...
    monkeypatch.setenv("OPENHUB_CIRCUIT_BREAKER_FAILURE_THRESHOLD", "10")
    monkeypatch.setenv("OPENHUB_CIRCUIT_BREAKER_RECOVERY_TIMEOUT_SECONDS", "120")
    settings = Settings()
...
def test_settings_backpressure_max_concurrent_requests_from_env(monkeypatch):
    monkeypatch.setenv("OPENHUB_BACKPRESSURE_MAX_CONCURRENT_REQUESTS", "50")
    settings = Settings()
...
def test_settings_split_bulkhead_env_overrides_are_removed(monkeypatch):
    ...
    monkeypatch.setenv("OPENHUB_BULKHEAD_PROXY_HTTP_LIMIT", "40")
    monkeypatch.setenv("OPENHUB_BULKHEAD_PROXY_WEBSOCKET_LIMIT", "25")
    monkeypatch.setenv("OPENHUB_BULKHEAD_PROXY_COMPACT_LIMIT", "8")
    settings = Settings()
...
def test_settings_work_admission_limits_from_env(monkeypatch):
    monkeypatch.setenv("OPENHUB_PROXY_TOKEN_REFRESH_LIMIT", "7")
    monkeypatch.setenv("OPENHUB_PROXY_UPSTREAM_WEBSOCKET_CONNECT_LIMIT", "9")
    monkeypatch.setenv("OPENHUB_PROXY_RESPONSE_CREATE_LIMIT", "11")
    monkeypatch.setenv("OPENHUB_PROXY_COMPACT_RESPONSE_CREATE_LIMIT", "3")
    settings = Settings()
...
def test_settings_upstream_websocket_proxy_env_defaults_to_direct_when_unset(monkeypatch):
    ...
    monkeypatch.delenv("OPENHUB_UPSTREAM_WEBSOCKET_TRUST_ENV", raising=False)
...
def test_settings_upstream_websocket_proxy_env_auto_enables_when_dotenv_proxy_is_present(monkeypatch, tmp_path):
    ...
    for name in (
        "https_proxy",
        "HTTPS_PROXY",
        "OPENHUB_UPSTREAM_WEBSOCKET_TRUST_ENV",
    ):
        monkeypatch.delenv(name, raising=False)
...
def test_settings_upstream_websocket_proxy_env_can_be_explicitly_disabled(monkeypatch):
    monkeypatch.setenv("https_proxy", "http://127.0.0.1:7890")
    monkeypatch.setenv("OPENHUB_UPSTREAM_WEBSOCKET_TRUST_ENV", "false")
...
def test_settings_workers_per_instance_defaults_to_one(monkeypatch):
    ...
    monkeypatch.delenv("OPENHUB_WORKERS_PER_INSTANCE", raising=False)
...
def test_settings_workers_per_instance_explicit_one_is_accepted(monkeypatch):
    monkeypatch.setenv("OPENHUB_WORKERS_PER_INSTANCE", "1")
...
def test_settings_rejects_multiple_workers_per_instance(monkeypatch):
    ...
    monkeypatch.setenv("OPENHUB_WORKERS_PER_INSTANCE", "2")

    with pytest.raises(ValidationError) as exc_info:
        Settings()

    message = str(exc_info.value)
    assert "OPENHUB_WORKERS_PER_INSTANCE" in message
