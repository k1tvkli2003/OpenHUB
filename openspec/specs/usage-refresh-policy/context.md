...
This context explains how openhub derives an account's usage and status, and
how to diagnose disagreements between openhub and Codex Desktop or the Codex
CLI quota pill.

openhub treats `/wham/usage` as the source of truth for account usage. Other
...
openhub refreshes account usage by calling:
...
There is no manual reset step inside openhub. Recovery is driven by the next
...
afterwards. openhub mirrors `/wham/usage` during that window, so the account
...
- Wait first. The next request through that account usually wakes the upstream
  rate limiter; openhub auto-recovers on the next refresh tick after the
...
- Do not manually flip the openhub account state to `ACTIVE` while
...
To confirm that the disagreement is upstream rather than openhub's mirror,
call `/wham/usage` directly with the same account token openhub is using:

```bash
ACCESS_TOKEN=...
ACCOUNT_ID=...   # chatgpt-account-id UUID, not openhub's id
...
shows the account as reset, openhub has nothing fresher to mirror. The account
...
- [#676 - initial bug report on `/wham/usage` vs. Settings UI divergence](https://github.com/k1tvkli2003/OpenHUB/issues/676)
- [#677 - dashboard per-account force-probe action](https://github.com/k1tvkli2003/OpenHUB/issues/677)
