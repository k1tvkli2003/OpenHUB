...
settings-load time: which `OPENHUB_*` values exist, which are deliberately
...
openhub captures the incoming ASGI client before delegating once to Uvicorn's
...
2. **Retire the retention env aliases.**
   `OPENHUB_REQUEST_LOG_RETENTION_DAYS` and
   `OPENHUB_USAGE_HISTORY_RETENTION_DAYS` are deprecated one-release
...
before phase 1; phases 1-4 removed 52 of them (plus adding `OPENHUB_TRACE`).
...
- OAuth protocol identity (6): `OPENHUB_AUTH_BASE_URL`
  (`https://auth.openai.com`), `OPENHUB_OAUTH_CLIENT_ID`
  (`app_EMoamEEZ73f0CkXaXp7hrann`), `OPENHUB_OAUTH_ORIGINATOR`
  (`codex_chatgpt_desktop`), `OPENHUB_OAUTH_SCOPE`
  (`openid profile email`), `OPENHUB_OAUTH_REDIRECT_URI`
  (`http://localhost:1455/auth/callback`), `OPENHUB_OAUTH_CALLBACK_PORT`
...
- Auth guardian tuning (7): interval 21600, max refresh age 43200, batch
  ...
  `OPENHUB_AUTH_GUARDIAN_ENABLED` remains the single switch.
- Debug log booleans (6): the `OPENHUB_LOG_PROXY_*` /
  `OPENHUB_LOG_UPSTREAM_*` booleans became `OPENHUB_TRACE` channels
...
- Bulkhead per-class overrides (3): http/websocket/compact limits always
  derive from `OPENHUB_BULKHEAD_PROXY_LIMIT` (http = websocket = proxy
...
- Token-refresh claim polling (2): wait 8.0 s, poll 0.25 s — constants in
  ...
  `OPENHUB_TOKEN_REFRESH_CLAIM_TTL_SECONDS` stays: its floor validation
...
- Codex client fingerprint (3): OS `Mac OS 26.5.0`, arch `arm64`,
  ...
  `OPENHUB_MODEL_REGISTRY_CLIENT_VERSION` bumps (which stays a setting:
...
- Memory warning threshold (1): derived as 80% of
  `OPENHUB_MEMORY_REJECT_THRESHOLD_MB` in
  ...
  anyway. `OPENHUB_MEMORY_REJECT_THRESHOLD_MB` stays: it is the one
...
- Images internals (2): host model fixed to `gpt-5.5`
  ...
  `OPENHUB_IMAGES_DEFAULT_MODEL` stays: the public API contract for
...
- DB pool tuning (4): background pool size / max overflow always derive
  ...
  `OPENHUB_DATABASE_POOL_SIZE` / `OPENHUB_DATABASE_MAX_OVERFLOW` stay:
...
- Soft-drain/probe thresholds (6): drain at 85%/90%, error window 60 s /
  ...
  constant defaults. `OPENHUB_SOFT_DRAIN_ENABLED` and
  `OPENHUB_DETERMINISTIC_FAILOVER_ENABLED` stay as the subsystem
  switches.
...
with `OPENHUB_TRACE=upstream_payload`. Startup never fails because of a
