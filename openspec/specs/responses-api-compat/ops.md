...
3. Does `openhub` preserve the same result when `Codex CLI` uses websocket transport through the proxy?

Use this runbook when investigating `fast` tier behavior for `Codex CLI`.

## Preconditions

- Repo path: `/home/egor/services/openhub-defin85`
- Python env: `.venv`
- DB connection is configured in `.env.local`
- The target account is already imported into `openhub`
...
```bash
PGPASSWORD='p-123456' psql -h 127.0.0.1 -U root -d openhub -c "select email,plan_type,status,chatgpt_account_id from accounts where email='TARGET_EMAIL';"
```
...
This probe bypasses `openhub` selection and measures what the upstream returns for one imported account.

Run:

```bash
set -a && source /home/egor/services/openhub-defin85/.env.local && set +a && cd /home/egor/services/openhub-defin85 && .venv/bin/python - <<'PY'
import asyncio, json
...
## Step 3: Verify `Codex CLI` Through Local `openhub`

Start a local proxy instance on a spare port:

```bash
cd /home/egor/services/openhub-defin85 && env OPENHUB_USAGE_REFRESH_ENABLED=false OPENHUB_MODEL_REGISTRY_ENABLED=false .venv/bin/fastapi run app/main.py --host 127.0.0.1 --port 2460 --no-proxy-headers
```
...
model_provider = "openhub-ws"
service_tier = "fast"

[model_providers.openhub-ws]
name = "OpenAI"
...
```bash
HOME="$tmp_home" RUST_LOG=debug codex exec --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox -C /home/egor/services/openhub-defin85 "Reply with OK only." > /tmp/codex-ws-run.out 2> /tmp/codex-ws-run.err
```
...
Check the latest request log written by `openhub`:

```bash
PGPASSWORD='p-123456' psql -h 127.0.0.1 -U root -d openhub -c "select requested_at,account_id,request_id,model,service_tier,status,error_code,error_message from request_logs order by requested_at desc limit 5;"
```
...
- compare them with the local probe or `openhub`
...
For stable HTTP bridge continuity across repeated calls, the same logical bridge key must reach the same `openhub` instance.
...
- Front proxies must size the HTTP path for the client's websocket→HTTP fallback and for remote-compaction POSTs, which carry full history. For nginx: `client_max_body_size 128m;` (matching openhub's own cap), plus websocket upgrade passthrough (`proxy_http_version 1.1;`, `proxy_set_header Upgrade $http_upgrade;`, `proxy_set_header Connection "upgrade";`) and a `proxy_read_timeout` comfortably above idle turn gaps (websocket connections live up to 60 minutes).
