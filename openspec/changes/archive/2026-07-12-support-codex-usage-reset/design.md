...
openhub already validates ChatGPT usage callers by calling upstream usage with
...
- Refresh openhub's local usage snapshot after upstream reports a successful or idempotently successful reset.

**Non-Goals:**

- Do not create local reset-credit accounting.
- Do not let openhub API-key callers consume ChatGPT account reset credits.
- Do not change aggregate usage-window semantics.

## Decisions

- Upstream ChatGPT remains the source of truth for reset-credit availability and consume outcome. openhub only preserves the availability summary and returns the upstream consume response.
...
- A successful upstream reset followed by local refresh failure could leave openhub briefly stale -> the next scheduled usage refresh still reconciles from upstream.
