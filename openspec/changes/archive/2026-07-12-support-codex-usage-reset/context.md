...
openhub should not maintain its own earned-reset balance. Upstream ChatGPT is
the authority for both availability and consume outcome. openhub only validates
...
action, openhub posts a generated `redeem_request_id` upstream using the
selected account tokens, openhub fetches upstream usage for the selected
...
Example upstream response preserved by openhub:
