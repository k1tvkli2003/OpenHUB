...
- Add `frontend/src/i18n/index.ts` that initialises `i18next` with `en` and `zh-CN` resources, persists the user's choice in `localStorage` under `openhub-language`, and falls back to `en` when the detected language is unsupported.
...
- The chosen language is persisted in `localStorage` under `openhub-language`. Clearing site data resets to detection.
