...
- Harden the bus: `bump()` retries transient failures with backoff and reports final failure via ERROR log + `openhub_cache_invalidation_bump_failures_total{namespace}`; new coalescing `request_bump()` keeps failed namespaces pending across poll cycles; poll failures escalate to WARNING after 3 / ERROR after 10 consecutive with `openhub_cache_invalidation_poll_failures_total`.
