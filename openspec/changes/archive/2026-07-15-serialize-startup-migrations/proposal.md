...
- New setting `database_migration_lock_timeout_seconds` (default `300.0`, env `OPENHUB_DATABASE_MIGRATION_LOCK_TIMEOUT_SECONDS`); on timeout, a `TimeoutError` names the migration lock and the setting (startup honors `database_migrations_fail_fast` as today; the CLI always fails).
