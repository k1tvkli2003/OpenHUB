...
- [x] 2.1 Add `database_migration_lock_timeout_seconds: float = 300.0` (env `OPENHUB_DATABASE_MIGRATION_LOCK_TIMEOUT_SECONDS`) to `app/core/config/settings.py`.
- [x] 2.2 Implement `app/db/migration_lock.py`: `migration_lock(sync_database_url, *, timeout_seconds)` — PostgreSQL `pg_try_advisory_lock(hashtext('openhub:migrations'))` poll loop on a dedicated AUTOCOMMIT connection with `pg_advisory_unlock` + dispose in `finally`; file-backed SQLite `BEGIN IMMEDIATE` poll loop on a `<db_path>.migrate-lock` sentinel connection; in-memory/other dialects no-op; INFO wait logging every ~10s; `TimeoutError` naming the lock and the setting.
...
- [x] 4.1 Concurrent-upgrade race: two barrier-aligned `run_upgrade` calls on a fresh database (SQLite tmp file; PostgreSQL when `OPENHUB_TEST_DATABASE_URL` is set) — both succeed, single head row, no duplicate-object errors.
