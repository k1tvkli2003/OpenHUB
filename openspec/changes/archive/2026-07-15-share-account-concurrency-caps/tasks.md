...
- [x] 6. Metrics: `openhub_cap_partition_replicas` gauge (multiprocess mode `livemax`, so dead workers do not pin a stale higher count) in `app/core/metrics/prometheus.py`; info-level rebalance log
