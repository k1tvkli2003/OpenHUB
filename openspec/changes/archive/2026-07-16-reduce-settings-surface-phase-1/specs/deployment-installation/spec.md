...
callback port) MUST be fixed constants: they identify openhub to OpenAI
exactly like the Codex CLI, and changing any of them breaks login. When a
previously supported `OPENHUB_*` setting is removed from the configuration
...
by the single `OPENHUB_TRACE` comma-separated channel list, whose empty
