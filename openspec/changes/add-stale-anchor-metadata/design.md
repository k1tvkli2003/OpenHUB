...
- Derive `previous_response_source` from request-state flags: `proxy_injected` when openhub rewrote the upstream body by trimming/replacing a client full resend, `client_supplied` when the inbound payload already carried `previous_response_id`, and `unknown` only when state is insufficient.
