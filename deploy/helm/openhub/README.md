...
helm upgrade --install openhub deploy/helm/openhub/ \
  -f deploy/helm/openhub/values-bundled.yaml \
  --set postgresql.auth.password=change-me
```
...
```bash
helm upgrade --install openhub deploy/helm/openhub/ \
  -f deploy/helm/openhub/values-external-db.yaml \
  --set externalDatabase.url='postgresql+asyncpg://user:pass@db.example.com:5432/openhub'
```
...
```bash
helm upgrade --install openhub deploy/helm/openhub/ \
  -f deploy/helm/openhub/values-external-secrets.yaml \
  --set externalSecrets.secretStoreRef.name=my-store
```
...
externalSecrets:
  ...
  remoteRefs:
    databaseUrl:
      key: /apps/openhub/DATABASE_URL
      property: ""
    encryptionKey:
      key: /apps/openhub/ENCRYPTION_KEY
      property: ""
```
...
When `externalDatabase.existingSecret` is set and `auth.existingSecret` is not, the chart-managed app secret contains only the encryption key; the StatefulSet reads `OPENHUB_DATABASE_URL` from the external DB secret.
...
- StatefulSet name: `<release>-openhub-workload`
- `serviceName: <release>-openhub-bridge` on the StatefulSet
- `OPENHUB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID=$(POD_NAME)`
- `OPENHUB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL=http://$(POD_NAME).<headless-service>.$(POD_NAMESPACE).svc.<clusterDomain>:2455`
...
```yaml
config:
  sessionBridgeAdvertiseBaseUrl: "http://$(POD_NAME).openhub-bridge.default.svc.cluster.local:2455"
```

Replace `openhub-bridge`, `default`, and `cluster.local` with your headless service name (`<release>-bridge` by default), namespace, and cluster domain. For a release named `openhub` the first pod then advertises `http://openhub-workload-0.openhub-bridge.default.svc.cluster.local:2455`.

When `networkPolicy.enabled=true`, the chart also allows port `2455` traffic between openhub pods so owner handoff can work without extra rules.
...
```yaml
config:
  sessionBridgeInstanceRing: "openhub-workload-0,openhub-workload-1"
```
...
custom rules, the chart preserves their order and adds the openhub Service as
...
```bash
kubectl port-forward svc/openhub 2455:2455 -n <namespace>
...
```bash
kubectl describe pod -l app.kubernetes.io/name=openhub -n <namespace>
